<template>
  <div class="space-y-6">
    <PageTitle
      :title="'奖品管理'"
      :subTitle="activityName ? `活动：${activityName}` : undefined"
    />
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
      <div class="text-sm text-muted-foreground">
        ...
        <template v-if="strategy !== 'guaranteed'">
          ，概率总和 {{ (probabilityTotal * 100).toFixed(2) }}%<span v-if="probabilityTotal > 1" class="text-red-600 font-medium">（超过 100%，需调整）</span>
        </template>
      </div>
    ...
    <Dialog :open="dialogOpen" @update:open="handleDialogOpenChange">
      <DialogContent class="max-w-lg mx-4">
        <DialogHeader>
          ...
          <DialogDescription>
            {{ isEditing ? '修改奖品信息，保存后立即生效' : `为活动「${activityName || ''}」添加新奖品` }}
          </DialogDescription>
        </DialogHeader>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField v-slot="{ componentField }" name="total_quantity">
              <FormItem>
                ...
                <FormDescription>奖品总库存，剩余数量随已中出数量自动扣减</FormDescription>
                <FormMessage />
              </FormItem>
            </FormField>
